...
import { User, Users, CreditCard, Key, CalendarClock } from "lucide-react";
...
export default function AccountLayout({ children }: AccountLayoutProps) {
  ...
  const tabs = [
    { value: "profile", label: "Profil", icon: <User className="h-4 w-4 mr-2" />, href: "/account/profile" },
    ...
    { value: "availability", label: "Disponibilité", icon: <CalendarClock className="h-4 w-4 mr-2" />, href: "/account/availability" }
  ];
    ...
    <AppLayout>
      <div className="space-y-6">
        <Breadcrumb className="mb-2">
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink href="/calendar">Tableau de bord</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href="/account/profile">Compte</BreadcrumbLink>
            </BreadcrumbItem>
        ...
        <Tabs value={activeTab} className="w-full">
          <TabsList className="w-full md:w-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
            {tabs.map(tab => (
              <TabsTrigger 
                key={tab.value} 
                value={tab.value} 
